...
class ContentUpdater {
  ...
  async init() {
    ...
    console.log(
      `🔑 TMDb API Key: ${this.tmdbApiKey ? "Configurado" : "No encontrado"}`
    );
    console.log(
      `🔑 OMDb API Key: ${this.omdbApiKey ? "Configurado" : "No encontrado"}\n`
    );
  }
      ...
      if (response.data && response.data.Response === "True") {
        return {
          ...
          runtime: response.data.Runtime
            ? parseInt(response.data.Runtime.replace(/\D/g, ""))
            : null,
  ...
  async updateContentItem(item) {
    ...
    try {
      ...
      let omdbData = null;
      if (tmdbData.imdb_id) {
        omdbData = await this.searchOMDb(
          item.title,
          item.year,
          tmdbData.imdb_id
        );
  ...
  async updateAllIncompleteContent() {
    try {
      ...
      console.log(
        `📊 Contenido para actualizar: ${incompleteContent.length} elementos\n`
      );
      ...
      console.log(
        `📊 Total procesados: ${this.updated + this.skipped + this.errors}`
      );
      console.log("");

      if (this.updated > 0) {
        console.log(
          "🎉 ¡Actualización completada! El contenido ahora debería mostrar:"
        );
